...
public partial class Form1 : Form
{
    ...
    private void ToggleSubClassInputs(bool enabled)
    {
        ...
        txtSubCreatedBy.Enabled = enabled;
        txtSubModifiedBy.Enabled = enabled;
    ...
    private void ToggleEnrollmentInputs(bool enabled)
    {
        ...
        txtEnrCreatedBy.Enabled = enabled;
        txtEnrModifiedBy.Enabled = enabled;
    ...
    private void LoadSubClasses()
    {
        ...
        if (response.IsSuccess)
        {
            ...
            try
            {
                ...
                foreach (var sc in response.SubClasses)
                {
                    // Match based on characteristics to find the correct ID
                    ...
                    if (match != null)
                    {
                        sc.SubClassId = match.SubClassId;
                    }
                }
            }
            catch (Exception ex) 
            ...
            dgvSubClasses.DataSource = response.SubClasses;
    ...
    private void btnClearSubClass_Click(object sender, EventArgs e)
    {
        ...
        txtSubCreatedBy.Text = "Admin";
        txtSubModifiedBy.Text = "Admin";
    ...
    private void btnSaveSubClass_Click(object sender, EventArgs e)
    {
        if (string.IsNullOrWhiteSpace(txtSelectedSubClassId.Text))
        {
            ...
            var req = new SubClassCreateRequestModel
            {
                ...
                CreatedBy = string.IsNullOrWhiteSpace(txtSubCreatedBy.Text) ? "Admin" : txtSubCreatedBy.Text,
                ModifiedBy = string.IsNullOrWhiteSpace(txtSubModifiedBy.Text) ? "Admin" : txtSubModifiedBy.Text
            };
        ...
        else
        {
            ...
            if (int.TryParse(txtSelectedSubClassId.Text, out int id))
            {
                var req = new SubClassPatchRequestModel
                {
                    ...
                    OpenDate = DateOnly.FromDateTime(dtpOpenDate.Value),
                    OpenTime = TimeOnly.FromDateTime(dtpOpenTime.Value)
                    // Note: ModifiedBy is missing from SubClassPatchRequestModel by design in Domain layer.
                };
    ...
    private void dgvSubClasses_CellClick(object sender, DataGridViewCellEventArgs e)
    {
        if (e.RowIndex >= 0)
        {
            ...
            if (dgvSubClasses.Columns.Contains("EditColumn") && e.ColumnIndex == dgvSubClasses.Columns["EditColumn"].Index)
            {
                ...
                if (dgvSubClasses.Columns.Contains("CreatedBy"))
                    txtSubCreatedBy.Text = row.Cells["CreatedBy"].Value?.ToString();
                
                if (dgvSubClasses.Columns.Contains("ModifiedBy"))
                    txtSubModifiedBy.Text = row.Cells["ModifiedBy"].Value?.ToString();
    ...
    private void LoadEnrollments()
    {
        ...
        if (response.IsSuccess)
        {
            dgvEnrollments.DataSource = response.Enrollments;
    ...
    private void btnClearEnrollment_Click(object sender, EventArgs e)
    {
        ...
        txtEnrCreatedBy.Text = "Admin";
        txtEnrModifiedBy.Text = "Admin";
    ...
    private void btnSaveEnrollment_Click(object sender, EventArgs e)
    {
        if (string.IsNullOrWhiteSpace(txtSelectedEnrollmentId.Text))
        {
            ...
            var req = new EnrollmentCreateRequestModel
            {
                ...
                CreatedBy = string.IsNullOrWhiteSpace(txtEnrCreatedBy.Text) ? "Admin" : txtEnrCreatedBy.Text,
                ModifiedBy = string.IsNullOrWhiteSpace(txtEnrModifiedBy.Text) ? "Admin" : txtEnrModifiedBy.Text
            };
    ...
    private void dgvEnrollments_CellClick(object sender, DataGridViewCellEventArgs e)
    {
        if (e.RowIndex >= 0)
        {
            ...
            if (dgvEnrollments.Columns.Contains("CreatedBy"))
                txtEnrCreatedBy.Text = row.Cells["CreatedBy"].Value?.ToString();
                
            if (dgvEnrollments.Columns.Contains("ModifiedBy"))
                txtEnrModifiedBy.Text = row.Cells["ModifiedBy"].Value?.ToString();
